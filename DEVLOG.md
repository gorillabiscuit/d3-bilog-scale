...
- **[10:22]** Fixed slider snapping linear window to center after pan — slider input now preserves qCenter=(manualQLo+manualQHi)/2 and domainCenter=(manualXLo+manualXHi)/2; only half-width changes (derived from windowQuantile on current slider value); window expands/contracts symmetrically around wherever the user panned
